...
export class App implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const arcoItems: MenuItem[] = [
      ...(isTechnician ? [{label: 'Gestionar Solicitudes ARCO', icon: 'pi pi-cog', routerLink: '/arco-manage'
      }] : []),
      ...(isApplicant ? [{label: 'Generar Solicitud ARCO', icon: 'pi pi-plus', routerLink: '/arco-request'
      }] : [])
    ];
    const dashboardItems: MenuItem[] = isTechnician ? [
      {
        label: 'Indicadores y KPIs',
    ...
    this.items = [
      ...
      ...(dashboardItems.length > 0 ? [{label: 'Dashboard', icon: 'pi pi-chart-line', items: dashboardItems
      }] : [])
    ];
  }
